...
大多数 UI 框架（包括 React v17 版本）都遵从同步渲染的流程，所渲染的 UI 也总是一致的。但在 React v18 上增加了并发机制，程序并不一定执行下去，会有中断的可能。
...
之后我们对 useSyncExternalStore 的源码进行分析，为了保证 store 状态一致，会通过同步阻塞渲染，强制更新，共采取了三道保险：
